...
/// `SpeakUp --screenshots <dir>` renders the pictures the README shows into
...
/// microphone or the engine, so it can run beside a copy of SpeakUp that is
...
enum Screenshots {
    ...
    /// SpeakUp, which is why the script runs the binary directly rather
    /// than through `open`.
...
private struct StylesFigure: View {
    ...
    /// A slice of menu bar with the live wave glyph where SpeakUp sits.
