...
| Interrupted press | A non-chord key within 1 s of the chord press cancels the recording without transcribing | Cmd+C, Cmd+V and Cmd+Tab start with the same modifier as the default chord; the overlay waits 150 ms before showing so those never flash it |
